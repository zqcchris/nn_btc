...
from exchange_market.binance.lib.utils import get_timestamp
from exchange_market.binance.lib.utils import cleanNoneValue
from exchange_market.binance.lib.utils import encoded_string
from exchange_market.binance.lib.utils import check_required_parameter
from exchange_market.binance.constant import BASE_URL_SPOT_REST
from exchange_market.binance.accounts.objects import AccountBinance
...
class SpotAPI(object):
    ...
    def __init__(
        ...
        if base_url:
            ...
            self.base_url = BASE_URL_SPOT_REST
    ...
    def sign_request(self, http_method, url_path, payload={}, account: AccountBinance = None):
        payload["timestamp"] = get_timestamp()
        query_string = self._prepare_params(payload)
        signature = self._get_sign(query_string, account=account)
        payload["signature"] = signature
        return self.send_request(http_method, url_path, payload, account=account)
    ...
    def send_request(self, http_method, url_path, payload={}, account: AccountBinance = None):
    ...
    def _get_sign(self, data, account: AccountBinance = None):
    ...
    def _dispatch_request(self, http_method, account: AccountBinance = None):
        if account:
            self.session.headers.update(
                {
                    "Content-Type": "application/json;charset=utf-8",
